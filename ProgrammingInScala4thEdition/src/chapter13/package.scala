 ...
 *               而且需要注意创建之后第一行不要加 package chapter13, 否则也报错…… 不过文件本身不报，比较神奇。
 *               如果加了 package chapter13 的话，这样编译后的类文件在包目录下创建了一个和类名相同的目录，下面才是 package.class。
 *               所以在包外使用 import chapter13.showFruit 是无效的，找不到。Chapter13Test 里面相当于是普通的包内调用，所以正常)
 * @ModifiedBy: zhuxi
 */
