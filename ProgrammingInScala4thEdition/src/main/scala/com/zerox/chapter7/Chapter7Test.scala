...
object Chapter7Test {
   ...
   */
  def test7_3(): Unit = {
    val filesHere = (new File("./ProgrammingInScala4thEdition/src/main/scala/com/zerox/chapter6")).listFiles
    for (file <- filesHere)
      println(file)
    for (i <- 1 to 4)
    ...
    val forLineLengths =
      for {
        file <- (new File("./ProgrammingInScala4thEdition/src/main/scala/com/zerox/chapter7")).listFiles
        if file.getName.endsWith(".scala")
        line <- fileLines(file)
        trimmed = line.trim
