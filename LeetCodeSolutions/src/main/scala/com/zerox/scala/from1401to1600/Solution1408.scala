...
/**
 * @author zhuxi
 * @since 2022/8/4 18:47
 * @note
 * 1408. 数组中的字符串匹配 | 难度：简单 | 标签：字符串、字符串匹配
...
object Solution1408 {
  ...
  def stringMatching(words: Array[String]): List[String] = {
    val sort = words.sortBy(_.length)
    val set = new scala.collection.mutable.HashSet[String]
    for (i <- sort.indices; j <- (i + 1) until sort.length if !set(sort(i)) && sort(j).contains(sort(i))) {
      set += sort(i)
    }
    set.toList
  }
