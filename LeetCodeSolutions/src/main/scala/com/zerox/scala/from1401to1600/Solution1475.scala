 ...
 * @since 2022/9/1 9:45
 * @note
 * 1475. 商品折扣后的最终价格 | 难度：简单 | 标签：栈、数组、单调栈
...
object Solution1475 {
   ...
   * 内存消耗：55.4 MB, 在所有 Scala 提交中击败了 100.00% 的用户
   * 通过测试用例：103 / 103
   *
   * @param prices
   * @return
  ...
  def finalPrices(prices: Array[Int]): Array[Int] = {
    ...
    val res = new Array[Int](prices.length)
    for (i <- prices.indices.reverse) {
      while (stack.nonEmpty && stack.head > prices(i)) {
        stack.pop()
      }
      res(i) = if (stack.isEmpty) prices(i) else prices(i) - stack.head
      stack.push(prices(i))
    }
    res
  }
