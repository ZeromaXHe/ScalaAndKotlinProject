...
/**
 * @author ZeromaXHe
 * @since 2022/8/21 9:39
 * @note
 * 1455. 检查单词是否为句中其他单词的前缀 | 难度：简单 | 标签：字符串、字符串匹配
...
object Solution1455 {
  /**
   * 执行用时：444 ms, 在所有 Scala 提交中击败了 100.00% 的用户
  ...
  def isPrefixOfWord(sentence: String, searchWord: String): Int = {
    sentence.trim().split("\\s+").zipWithIndex.find(_._1.startsWith(searchWord)).getOrElse(("", -2))._2 + 1
  }
}
