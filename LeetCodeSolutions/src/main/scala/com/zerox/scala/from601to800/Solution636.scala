...
/**
 * @author zhuxi
 * @since 2022/8/5 16:59
 * @note
 * 636. 函数的独占时间 | 难度：中等 | 标签：栈、数组
...
object Solution636 {
  ...
  def exclusiveTime(n: Int, logs: List[String]): Array[Int] = {
    ...
    for (log <- logs) {
      val strs = log.split(":")
      val task = Integer.parseInt(strs(0))
      val now = Integer.parseInt(strs(2))
      if ("start".equals(strs(1))) {
        if (stack.nonEmpty) result(stack.top(0)) += now - stack.top(1)
        stack.push(Array(task, now))
      } else {
        result(task) += now - stack.top(1) + 1
        stack.pop()
        if (stack.nonEmpty) stack.top(1) = now + 1
      }
    }
    result
  }
