...
/**
 * @author ZeromaXHe
 * @since 2022/9/12 12:18
 * @note
 * 1608. 特殊数组的特征值 | 难度：简单 | 标签：数组、二分查找、排序
...
object Solution1608 {
  def main(args: Array[String]): Unit = {
    println(specialArray(Array(0, 4, 3, 0, 4))) // 3
    println(specialArray(Array(3, 6, 7, 7, 0))) // -1
  }
  ...
  def specialArray(nums: Array[Int]): Int = {
    ...
    while (l < r) {
      val mid = l + (r - l) / 2
      if (sort.length - mid <= sort(mid) && (mid == 0 || sort(mid - 1) < sort.length - mid)) {
        return sort.length - mid
      } else if (sort.length - mid <= sort(mid)) r = mid
      else l = mid + 1
    }
    -1
  }
