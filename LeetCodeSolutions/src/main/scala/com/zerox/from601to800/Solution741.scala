...
/**
 * @author ZeromaXHe
 * @since 2022/7/10 12:53
 * @note
 * 741. 摘樱桃 | 难度：困难 | 标签：数组、动态规划、矩阵
...
object Solution741 {
  ...
  def cherryPickup(grid: Array[Array[Int]]): Int = {
    val n = grid.length
    val f = Array.fill(n) {
      Array.fill(n)(Int.MinValue)
    }
    f(0)(0) = grid(0)(0)
    for (k <- 1 until (n * 2 - 1);
         x1 <- Range(math.min(k, n - 1), math.max(k - n + 1, 0) - 1, -1);
         x2 <- Range(math.min(k, n - 1), x1 - 1, -1)) {
      val y1 = k - x1
      val y2 = k - x2
