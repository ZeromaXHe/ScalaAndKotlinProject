...
object Solution1926 {
    ...
    /**
     * 执行用时：424 ms, 在所有 Kotlin 提交中击败了 100.00% 的用户
     * 内存消耗：56 MB, 在所有 Kotlin 提交中击败了 100.00% 的用户
     * 通过测试用例：194 / 194
     */
    fun nearestExit(maze: Array<CharArray>, entrance: IntArray): Int {
        ...
        while (queue.isNotEmpty()) {
            ...
            val y = deq[1]
            val dist = deq[2]
            if (dist > 0 && (x == 0 || x == maze.size - 1 || y == 0 || y == maze[0].size - 1)) return dist
            if (x + 1 < maze.size && maze[x + 1][y] == '.') {
                queue.offer(intArrayOf(x + 1, y, dist + 1))
                maze[x + 1][y] = '*'
            }
            if (x - 1 >= 0 && maze[x - 1][y] == '.') {
                queue.offer(intArrayOf(x - 1, y, dist + 1))
            ...
            if (y + 1 < maze[0].size && maze[x][y + 1] == '.') {
                queue.offer(intArrayOf(x, y + 1, dist + 1))
                maze[x][y + 1] = '*'
            }
            if (y - 1 >= 0 && maze[x][y - 1] == '.') {
                queue.offer(intArrayOf(x, y - 1, dist + 1))
